    ...
    for (i, (op, tag)) in operations.iter().enumerate() {
      ...
      match opcode {
            ...
            0x21 | 0x23..=0x27 | 0x2A | 0x2B => {
              match (const_registers[tag.input_i(0)], const_registers[tag.input_i(1)]) {
                (Some(c1), Some(c2)) => {
                  ...
                  X64JIT::emit_store_constant(&mut ret, result, output, logging);
        ...
        0x23 => {
          ...
          match const_registers[tag.input_i(0)] {
            Some(addr) => {
              X64JIT::emit_optimized_read_word(&mut ret, *op, addr, logging);
            },
            None => {
              ret.push(self.compile_opcode(*op, logging).expect(""));
            },
        ...
        0x24 => {
          ...
          match const_registers[tag.input_i(0)] {
            Some(addr) => {
              X64JIT::emit_optimized_read_byte(&mut ret, *op, addr, logging);
            },
            None => {
              ret.push(self.compile_opcode(*op, logging).expect(""));
            },
        ...
        0x25 => {
          ...
          match const_registers[tag.input_i(0)] {
            Some(addr) => {
              X64JIT::emit_optimized_read_half(&mut ret, *op, addr, logging);
            },
            None => {
              ret.push(self.compile_opcode(*op, logging).expect(""));
            },
        ...
        0x28 => {
          match (const_registers[tag.input_i(0)], const_registers[tag.input_i(1)]) {
            (Some(addr), _) => {
              X64JIT::emit_optimized_write_byte(&mut ret, *op, addr, logging);
            },
            _ => {
              ret.push(self.compile_opcode(*op, logging).expect(""));
            },
        ...
        0x29 => {
          match (const_registers[tag.input_i(0)], const_registers[tag.input_i(1)]) {
            (Some(addr), _) => {
              X64JIT::emit_optimized_write_half(&mut ret, *op, addr, logging);
            },
            _ => {
              ret.push(self.compile_opcode(*op, logging).expect(""));
            },
        ...
        0x2B => {
          match (const_registers[tag.input_i(0)], const_registers[tag.input_i(1)]) {
            (Some(addr), _) => {
              X64JIT::emit_optimized_write_word(&mut ret, *op, addr, logging);
            },
            _ => {
              ret.push(self.compile_opcode(*op, logging).expect(""));
            },
