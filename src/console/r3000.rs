...
impl R3000 {
    ...
    pub fn nth_reg_mut(&mut self, idx: u32) -> Option<MutReg> {
        ...
        match idx {
            0 => None,
            _ => Some(MutReg::new(&mut self.registers[idx], Name::Rn(idx as u32))),
        }
    }
    ...
    pub fn flush_write_cache(
        &mut self, operations: &mut VecDeque<DelayedWrite>, modified_register: &mut Option<Name>,
    ) {
        match operations.pop_front() {
            Some(write) => match modified_register {
                Some(name) => {
