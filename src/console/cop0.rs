...
impl Cop0 {
  ...
  pub fn nth_data_reg_mut(&mut self, idx: u32) -> Option<&mut u32> {
    match idx {
      12 => {
        Some(&mut self.registers[Cop0::IDX_R12])
      },
      13 => {
        Some(&mut self.registers[Cop0::IDX_R13])
      },
      14 => {
        Some(&mut self.registers[Cop0::IDX_R14])
      },
      _ => {
  ...
  pub fn execute_command(&mut self, imm25: u32) -> Option<u32> {
    ...
    if imm25 == 0x0000_0010 {
      ...
      self.registers[Cop0::IDX_R12].clear_mask(0x0f).set_mask(bits2_3).set_mask(bits4_5);
    }
    None
  }
  ...
  fn set_exception_cause(&mut self, cause: u32) {
    assert!(cause < 0x20);
    self.registers[Cop0::IDX_R13].clear(2).clear(3).clear(4).clear(5).clear(6).set_mask(cause << 2);
  }
  ...
  fn disable_interrupts(&mut self) {
    ...
    self.registers[Cop0::IDX_R12].clear(0).clear(2).set_mask(prev << 2);
  }
}
