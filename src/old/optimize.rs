...
impl CachingInterpreter {
  pub(super) fn compile_stub(&mut self, operations: &mut Vec<(u32, Insn)>, logging: bool) -> Vec<Box<dyn Fn(&mut Console)>> {
    let mut ret = Vec::new();
  ...
  fn emit_optimized_read_byte(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, op: u32, constant: u32, logging: bool) {
    let t = get_rt(op);
  ...
  fn emit_optimized_read_half(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, op: u32, constant: u32, logging: bool) {
    let t = get_rt(op);
    let imm16 = get_imm16(op).half_sign_extended();
    let constant_address = constant.wrapping_add(imm16);
    ret.push(Box::new(move |vm| {
  ...
  fn emit_optimized_read_word(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, op: u32, constant: u32, logging: bool) {
    let t = get_rt(op);
    let imm16 = get_imm16(op).half_sign_extended();
    let constant_address = constant.wrapping_add(imm16);
    ret.push(Box::new(move |vm| {
  ...
  fn emit_optimized_write_byte(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, op: u32, constant: u32, logging: bool) {
    let t = get_rt(op);
    let imm16 = get_imm16(op).half_sign_extended();
    let constant_address = constant.wrapping_add(imm16);
    ret.push(Box::new(move |vm| {
  ...
  fn emit_optimized_write_half(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, op: u32, constant: u32, logging: bool) {
    let t = get_rt(op);
    let imm16 = get_imm16(op).half_sign_extended();
    let constant_address = constant.wrapping_add(imm16);
    ret.push(Box::new(move |vm| {
  ...
  fn emit_optimized_write_word(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, op: u32, constant: u32, logging: bool) {
    let t = get_rt(op);
    let imm16 = get_imm16(op).half_sign_extended();
    let constant_address = constant.wrapping_add(imm16);
    ret.push(Box::new(move |vm| {
  ...
  fn emit_store_constant(ret: &mut Vec<Box<dyn Fn(&mut Console)>>, result: u32, name: u32, logging: bool) {
    ret.push(Box::new(move |vm| {
  ...
  pub(super) fn compile_optimized_stub(&mut self, operations: &mut Vec<(u32, Insn)>, logging: bool) -> Vec<Box<dyn Fn(&mut Console)>> {
