...
impl Insn {
    pub fn new(op: u32, offset: u32) -> Self {
        let (inputs, index, output) = match get_primary_field(op) {
            0x00 => {
                match get_secondary_field(op) {
                    ...
                    0x18 | 0x19 | 0x1A | 0x1B => {
                        //MULT, MULTU, DIV, DIVU
                        (
                            vec![get_rs(op), get_rt(op), R3000::HI_IDX, R3000::LO_IDX],
                            None,
                            None,
                        )
                    },
    ...
    pub fn has_branch_delay_slot(op: u32) -> bool {
        match get_primary_field(op) {
            0x00 => match get_secondary_field(op) {
                0x08 | 0x09 => true,
                _ => false,
            },
            0x01 => match get_rt(op) {
...
impl InsnRegisters for Vec<Insn> {
    ...
    fn registers_by_frequency(&self) -> Vec<MIPSRegister> {
        ...
        sorted_registers.sort_by(|x, y| {
            let comparison = registers_used
                .iter()
                .filter(|&z| z == y)
                .count()
                .cmp(&registers_used.iter().filter(|&z| z == x).count());
            match comparison {
